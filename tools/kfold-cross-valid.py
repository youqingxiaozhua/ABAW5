...
from mmcls.utils import collect_env, get_root_logger, load_json_log, auto_select_device
...
def train_single_fold(args, cfg, fold, distributed, seed):
    ...
    if cfg.data.train.type == 'ClassBalancedDataset':
        origin_train = cfg.data.train.dataset
        class_balanced = True
    else:
        origin_train = cfg.data.train

    # wrap the dataset cfg
    train_dataset = dict(
        type='KFoldDataset',
        fold=fold,
        dataset=origin_train,
    ...
    val_dataset = dict(
        ...
        dataset=copy.deepcopy(origin_train),
        num_splits=args.num_splits,
        seed=seed,
        test_mode=True,
    )
    ...
    if class_balanced:
        cfg.data.train.dataset = train_dataset
    else:
